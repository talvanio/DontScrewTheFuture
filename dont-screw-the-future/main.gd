...
func _process(delta: float) -> void:
	if player.current_health > 0:
		...
		# --- DISPARADOR DEL MINIJEFE ---
		if game_time >= boss_spawn_time and not boss_spawned:
			spawn_miniboss()
		# -------------------------------
	else:
		run_game_over()
...
# --- FUNCIÓN PARA INSTANCIAR AL MINIJEFE ---
func spawn_miniboss() -> void:
	boss_spawned = true
	if boss_scene:
		var boss = boss_scene.instantiate()
		
		# Centrar en X dinámicamente según la pantalla
		var viewport_size = get_viewport_rect().size
		boss.position.x = viewport_size.x / 2
		
		# El eje Y inicial de spawn (-100.0) se gestiona internamente en el script del miniboss
		add_child(boss)
